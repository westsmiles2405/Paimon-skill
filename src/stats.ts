/**
 * 派蒙数据统计 — 番茄钟 / 消息 / 保存 次数追踪
 */

import * as vscode from 'vscode';

export interface PaimonStats {
...
export class StatsTracker {
    private stats: PaimonStats;
    private static readonly KEY = 'paimon.stats';

    constructor(private readonly state: vscode.Memento) {
        this.stats = state.get<PaimonStats>(StatsTracker.KEY, {
            totalPomodoros: 0,
            totalMessages: 0,
            totalSaves: 0,
            todayPomodoros: 0,
    ...
    public get current(): PaimonStats {
        this.resetIfNewDay();
        return { ...this.stats };
    }
}
